...
import org.apache.hadoop.tools.mapred.CopyWithTempDirMapper.FileAction;
 ...
 */
public class RetriableFileCopyCommandWithTempDir extends RetriableCommand {

  private static Log LOG = LogFactory.getLog(RetriableFileCopyCommandWithTempDir.class);
   ...
   */
  public RetriableFileCopyCommandWithTempDir(String description, FileAction action) {
    super(description);
    this.action = action;
  }
   ...
   */
  public RetriableFileCopyCommandWithTempDir(boolean skipCrc, String description,
      FileAction action) {
  ...
  private Path getTmpFile(Path target, Mapper.Context context) {
    ...
    if (context.getConfiguration().
            get(DistCpConstants.CONF_LABEL_TARGET_TEMP_PATH).isEmpty()) {

        tempPath = context.getConfiguration().
                get(DistCpConstants.CONF_LABEL_TARGET_WORK_PATH);
        LOG.debug("temp not set, temp path: " + new Path(tempPath));
    } else {
        tempPath = context.getConfiguration().
                get(DistCpConstants.CONF_LABEL_TARGET_TEMP_PATH);
        LOG.debug("temp is set, temp path: " + new Path(tempPath));
    }
    Path targetWorkPath = new Path(tempPath);
