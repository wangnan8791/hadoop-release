...
public class CopyOutputFormat<K, V> extends TextOutputFormat<K, V> {
  ...
  public static void setTempDirectory(Job job, Path tempDirectory) {
	    job.getConfiguration().set(DistCpConstants.CONF_LABEL_TARGET_TEMP_PATH,
  ...
  private static Path getWorkingDirectory(Configuration conf) {
	...
	public static Path getTempDirectory(Job job) {
		return getTempDirectory(job.getConfiguration());
	}

	private static Path getTempDirectory(Configuration conf) {
		String tempDirectory = conf.get(DistCpConstants.CONF_LABEL_TARGET_TEMP_PATH);
		if (tempDirectory == null || tempDirectory.isEmpty()) {
			return null;
		} else {
			return new Path(tempDirectory);
		}
	}
